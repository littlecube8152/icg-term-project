...
bool Dielectric::scatter(const Ray &r_in, HitRecord &rec) const {
    ...
    glm::vec3 refracted = glm::refract(r_in.velocity(), rec.normal, real_eta);
    glm::vec3 reflected = glm::reflect(r_in.velocity(), rec.normal);
    if (glm::epsilonEqual(glm::length(refracted), 0.0f, kEpsilon)) {
        // total internal reflection
        rec.scattered = Ray(rec.p, reflected);
    } else {
        float cosine = glm::dot(-r_in.velocity(), rec.normal);
