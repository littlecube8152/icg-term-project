...
bool Dielectric::scatter(const Ray &r_in, HitRecord &rec) const {
    ...
    glm::vec3 refracted = glm::refract(glm::normalize((glm::vec3)r_in.velocity()), rec.normal, real_eta);
    ...
    if (glm::epsilonEqual(glm::length(refracted), 0.0f, kEpsilon)) {
        ...
        float cosine = glm::dot(-glm::normalize((glm::vec3)r_in.velocity()), rec.normal);
