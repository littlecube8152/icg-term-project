...
GLuint Camera::renderAsTexture(void) {
    ...
    for (GLuint x = 0; x < image_width; x++) {
        for (GLuint y = 0; y < image_height; y++) {
            ...
            GLfloat b = (GLfloat)(x + y) / (GLfloat)(image_width + image_height);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8UI, image_width, image_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, texture_data.data());
