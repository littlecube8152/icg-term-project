...
SceneRelativisticDopplerTest::SceneRelativisticDopplerTest(const ArgumentParser &options, float time_scale)
{
    camera = Camera((CameraConfig){
        .image_width = static_cast<unsigned>(options.getWidth()),
        .image_height = static_cast<unsigned>(options.getHeight()),
        .vfov = 90,
        .lookfrom = glm::vec3(-0.2, 10, -1),
        .lookat = glm::vec3(0.2, 9.5, 0),
    ...
    world.add(std::make_shared<Cube>(glm::vec3(1000.5f, -1000.5f, 1.0f), 2000.0f, material_ground));
    world.add(std::make_shared<Cube>(glm::vec3(-1000.5f, -1000.5f, 1.0f), 2000.0f, material_ground));
}